        ...
        try:
            ...
            with tab1:
                ...
                if "results" in st.session_state:
                    ...
                    # Generate and Download AI Report
                    def generate_ai_report(res_df, dataset_name):
                        from datetime import datetime
                        
                        report_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        critical_engines = res_df[res_df["Status"] == "Critical"]
                        warning_engines = res_df[res_df["Status"] == "Warning"]
                        healthy_engines = res_df[res_df["Status"] == "Healthy"]
                        
                        avg_rul = res_df["Predicted_RUL"].mean()
                        min_rul = res_df["Predicted_RUL"].min()
...
## 1. Executive Summary
The fleet consists of **{len(res_df)}** engines.
- **Average RUL:** {avg_rul:.2f} cycles
- **Minimum RUL:** {min_rul:.2f} cycles
- **Critical Engines:** {len(critical_engines)} ({len(critical_engines)/len(res_df)*100:.1f}%)
- **Warning Engines:** {len(warning_engines)} ({len(warning_engines)/len(res_df)*100:.1f}%)
- **Healthy Engines:** {len(healthy_engines)} ({len(healthy_engines)/len(res_df)*100:.1f}%)

## 2. Critical Alerts (Immediate Action Required)
The following engines have an RUL of less than 50 cycles and require immediate maintenance inspection:

| Unit ID | Predicted RUL |
| :--- | :--- |
"""
                        for _, row in critical_engines.iterrows():
                            report += f"| {int(row['unit'])} | {row['Predicted_RUL']:.2f} |\n"
                            
                        if len(critical_engines) == 0:
                            report += "No critical engines detected.\n"
                            
                        report += """
## 3. Recommendations
Based on the predictive analysis, the following actions are recommended:

### 🔴 Critical Priority
- **Schedule immediate maintenance** for all engines listed in the Critical Alerts section.
- **Inspect key components** (HPC, Fan) for signs of degradation.
- **Review operational logs** for these units to identify stress factors.

### 🟠 Warning Priority
- **Monitor closely** engines with RUL between 50 and 100 cycles.
- **Plan upcoming maintenance slots** to avoid operational disruption.
- **Optimize load** on these engines if possible to extend remaining life.

### 🟢 General Fleet Health
- Continue routine monitoring for healthy engines.
- Update the predictive model with new data every 50 cycles to maintain accuracy.

---
*Generated by Turbofan RUL Predictor AI*
"""
                        return report

                    with c2:
                        ai_report = generate_ai_report(results, selected_dataset)
                        st.download_button(
                            label="📄 Download AI Report (TXT)",
                            data=ai_report,
                            file_name=f'ai_maintenance_report_{selected_dataset}.txt',
                            mime='text/plain',
                        )
